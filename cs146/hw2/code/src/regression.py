...
class PolynomialRegression() :
    ...
    def fit_GD(self, X, y, eta=None,
        ...
        for t in range(tmax) :
            ...
            #self.coef_ = self.coef_ - eta* (np.dot(X.T, np.dot(X,self.coef_)) - np.dot(X.T,y))    
            # track error
            # hint: you cannot use self.predict(...) to make the predictions
            y_pred = np.dot(self.coef_, X.T)
            #y_pred = self.predict(X) # change this line
            err_list[t] = np.sum(np.power(y - y_pred, 2)) / float(n)
            self.coef_ -= 2*eta*np.dot((y_pred-y),X)
            ### ========== TODO : END ========== ###
    ...
    def cost(self, X, y) :
        ...
        cost= np.sum((np.dot(X, self.coef_) - y)**2)
    ...
    def rms_error(self, X, y) :
        ...
        error = ((((np.dot(self.coef_, X.T) - y)**2).sum())/n)**0.5
        ### ========== TODO : END ========== ###
        return error
...
def main() :
    ...
    for eta in [0.0407,0.01,0.001,0.0001]:
        time1= time.time()
        model.fit_GD(X=train_data.X, y=train_data.y,eta=eta,verbose=False)
    ...
    print('Investigating modified eta...')
    time1=time.time()
    ...
    plt.clf()
    plt.plot(ms, trainErrs, 'bs-',label="train erms")
    plt.plot(ms, testErrs, 'gs-', label="test erms")
