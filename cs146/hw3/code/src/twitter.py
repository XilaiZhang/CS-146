...
def select_param_linear(X, y, kf, metric="accuracy"):
    ...
    max=-1
    for c in C_range:
        new=cv_performance(SVC(C=c, kernel='linear'), X, y, kf, metric) 
        if new>max:
            max=new
            optimal=c
    return optimal
...
def main() :
    ...
    for metric in ['AUROC','accuracy', 'F1-Score' ]:
        ...
        lst = np.array([cv_performance(SVC(C=c, kernel='linear'), trainsetX, trainsety, kf, metric) for c in C_range])
        print(lst)
        print(select_param_linear(trainsetX, trainsety, kf, metric))
    ...
    for metric in ['accuracy', 'F1-Score', 'AUROC']:
        print(performance_test(classifier, testsetX, testsety, metric))
    ### ========== TODO : END ========== ###
